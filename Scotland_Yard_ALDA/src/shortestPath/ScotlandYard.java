...
public class ScotlandYard {
	...
	public static DirectedGraph<Integer> getGraph() throws FileNotFoundException {
		...
		Scanner in = new Scanner(new File("src/ScotlandYard_Kanten.txt"));
		// ...
		String line;
		Map<String, Integer> cost = new TreeMap<>();
		while (in.hasNextLine()) {
			line = in.nextLine();
			//String[] s = line.split("[^a-z^A-Z^0-9]+");
			...
			if (!sy_graph.containsEdge(u,v)) {
				sy_graph.addEdge(u, v, cost.get(s[2]));
				sy_graph.addEdge(v, u, cost.get(s[2]));
			} else if (sy_graph.getWeight(u,v) > cost.get(s[2])) {
				sy_graph.addEdge(u, v, cost.get(s[2]));
				sy_graph.addEdge(v, u, cost.get(s[2]));
			}
		}
	 ...
	 * @throws FileNotFoundException
	 */
	public static shortestPath.Heuristic<Integer> getHeuristic() throws FileNotFoundException {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		//shortestPath.Heuristic<Integer> syHeuristic = null; // Dijkstra
		Heuristic<Integer> syHeuristic = getHeuristic(); // A*

		shortestPath.ShortestPath<Integer> sySp = new ShortestPath<Integer>(syGraph,syHeuristic);
...
class ScotlandYardHeuristic<V> implements Heuristic<Integer> {
	...
	public ScotlandYardHeuristic() throws FileNotFoundException {
		// ...
		Scanner in = new Scanner(new File("src/ScotlandYard_Knoten.txt"));
		coord = new TreeMap<>();
		// ...
		String line;
		while (in.hasNextLine()) {
