...
public class DepthFirstOrder<V> {
    ...
    private void visitAllNodes() {
        for (V e : myGraph.getVertexSet()) {
            if (!preOrder.contains(e)) {
                visitDF(e, myGraph, preOrder);
                numberOfDFTrees++;
            }
        }
    }
    ...
    private void visitDF (V v, DirectedGraph<V> g, List<V> preOrder) {
        preOrder.add(v);
        for (V e : myGraph.getSuccessorVertexSet(v)) {
            if (!preOrder.contains(e)) {
                visitDF(e, g, preOrder);
            }
        }
        postOrder.add(v);
    }
